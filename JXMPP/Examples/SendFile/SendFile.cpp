...
class FileSender {
    public:
    FileSender(const JID& jid, const std::string& password, const JID& recipient, const boost::filesystem::path& file) : jid(jid), password(password), recipient(recipient), file(file), tracer(nullptr), client(std::unique_ptr<JXMPP::Client>(new JXMPP::Client(jid, password, &networkFactories))) {
    ...
    private:
        ...
        std::unique_ptr<JXMPP::Client> client;
};
