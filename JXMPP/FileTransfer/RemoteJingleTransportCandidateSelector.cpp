...
void RemoteJingleTransportCandidateSelector::tryNextCandidate() {
    if (candidates.empty()) {
        JXMPP_LOG(debug) << "No more candidates" << std::endl;
        ...
        candidates.pop();
        JXMPP_LOG(debug) << "Trying candidate " << lastCandidate.cid << std::endl;
        if ((lastCandidate.type == JingleS5BTransportPayload::Candidate::DirectType && options.isDirectAllowed()) ||
            ...
            JXMPP_LOG(debug) << "Can't handle this type of candidate" << std::endl;
            tryNextCandidate();
        }
    }
}
