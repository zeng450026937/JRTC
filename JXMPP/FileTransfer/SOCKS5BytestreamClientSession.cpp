...
void SOCKS5BytestreamClientSession::start() {
    ...
    JXMPP_LOG(debug) << "Trying to connect via TCP to " << addressPort.toString() << "." << std::endl;
...
void SOCKS5BytestreamClientSession::process() {
    JXMPP_LOG(debug) << "unprocessedData.size(): " << unprocessedData.size() << std::endl;
    ...
    switch(state) {
        ...
        case Authenticating:
            ...
            unprocessedData.clear();
            state = Ready;
            JXMPP_LOG(debug) << "session ready" << std::endl;
            // issue ready signal so the bytestream can be used for reading or writing
            weFailedTimeout->stop();
            onSessionReady(false);
            break;
        case Ready:
            JXMPP_LOG(debug) << "Received further data in Ready state." << std::endl;
            break;
        case Reading:
        case Writing:
        case Finished:
            JXMPP_LOG(debug) << "Unexpected receive of data. Current state: " << state << std::endl;
            JXMPP_LOG(debug) << "Data: " << Hexify::hexify(unprocessedData) << std::endl;
            unprocessedData.clear();
            //assert(false);
    }
}
...
void SOCKS5BytestreamClientSession::startReceiving(std::shared_ptr<WriteBytestream> writeStream) {
    if (state == Ready) {
        ...
        JXMPP_LOG(debug) << "Session isn't ready for transfer yet!" << std::endl;
    }
}
...
void SOCKS5BytestreamClientSession::handleConnectFinished(bool error) {
    ...
    if (error) {
        JXMPP_LOG(debug) << "Failed to connect via TCP to " << addressPort.toString() << "." << std::endl;
        finish(true);
    } else {
        JXMPP_LOG(debug) << "Successfully connected via TCP" << addressPort.toString() << "." << std::endl;
...
void SOCKS5BytestreamClientSession::handleDataRead(std::shared_ptr<SafeByteArray> data) {
    JXMPP_LOG(debug) << "state: " << state << " data.size() = " << data->size() << std::endl;
    if (state != Reading) {
...
void SOCKS5BytestreamClientSession::handleDisconnected(const boost::optional<Connection::Error>& error) {
    JXMPP_LOG(debug) << (error ? (error == Connection::ReadError ? "Read Error" : "Write Error") : "No Error") << std::endl;
    if (error) {
        finish(true);
    }
}
...
void SOCKS5BytestreamClientSession::handleWeFailedTimeout() {
    JXMPP_LOG(debug) << "Failed due to timeout!" << std::endl;
    finish(true);
}
