...
void DefaultFileTransferTransporter::startActivatingProxy(const JID& proxyServiceJID) {
    // activate proxy
    JXMPP_LOG(debug) << "Start activating proxy " << proxyServiceJID.toString() << " with sid = " << s5bSessionID << "." << std::endl;
...
std::shared_ptr<TransportSession> DefaultFileTransferTransporter::createLocalCandidateSession(
    ...
    if (candidate.type == JingleS5BTransportPayload::Candidate::ProxyType) {
        ...
        if (proxySession) {
            ...
            JXMPP_LOG(error) << "Failed obtaining proxy session with candidate JID " << candidate.jid << " and dstAddr " << getLocalCandidateSOCKS5DstAddr() << "." << std::endl;
        }
    }
...
std::string DefaultFileTransferTransporter::getSOCKS5DstAddr() const {
    ...
    if (role == Initiator) {
        ...
        JXMPP_LOG(debug) << "Initiator S5B DST.ADDR = " << s5bSessionID << " + " << initiator.toString() << " + " << responder.toString() << " : " << result << std::endl;
    }
    else {
        result = getResponderCandidateSOCKS5DstAddr();
        JXMPP_LOG(debug) << "Responder S5B DST.ADDR = " << s5bSessionID << " + " << responder.toString() << " + " << initiator.toString() << " : " << result << std::endl;
    }
    return result;
}
