...
{ \
    ...
    JXMPP_LOG(debug) << std::hex << function << ": status: 0x" << status << ": " << errorCode->message() << std::endl; \
}
...
bool CAPICertificate::checkIfSmartCardPresent () {
    if (!smartCardReaderName_.empty()) {
        ...
        switch (dwState) {
            case SCARD_ABSENT:
                JXMPP_LOG(debug) << "Card absent." << std::endl;
                break;
            case SCARD_PRESENT:
                JXMPP_LOG(debug) << "Card present." << std::endl;
                break;
            case SCARD_SWALLOWED:
                JXMPP_LOG(debug) << "Card swallowed." << std::endl;
                break;
            case SCARD_POWERED:
                JXMPP_LOG(debug) << "Card has power." << std::endl;
                break;
            case SCARD_NEGOTIABLE:
                JXMPP_LOG(debug) << "Card reset and waiting PTS negotiation." << std::endl;
                break;
            case SCARD_SPECIFIC:
                JXMPP_LOG(debug) << "Card has specific communication protocols set." << std::endl;
                break;
            default:
                JXMPP_LOG(debug) << "Unknown or unexpected card state." << std::endl;
                break;
        }
