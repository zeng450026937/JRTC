...
CFArrayRef CreateClientCertificateChainAsCFArrayRef(CertificateWithKey::ref key) {
    ...
    switch(securityError) {
        ...
        case errSecAuthFailed:
            ...
            JXMPP_LOG(warning) << "Invalid password." << std::endl;
            break;
        case errSecDecode:
            // Other decoding error.
            JXMPP_LOG(warning) << "PKCS12 decoding error." << std::endl;
            break;
        default:
            JXMPP_LOG(warning) << "Unknown error." << std::endl;
    }
...
SecureTransportContext::SecureTransportContext(bool checkCertificateRevocation) : state_(None), checkCertificateRevocation_(checkCertificateRevocation) {
    ...
    if (error != noErr) {
        JXMPP_LOG(error) << "Unable to set IO functions to SSL context." << std::endl;
        sslContext_.reset();
    }

    error = SSLSetConnection(sslContext_.get(), this);
    if (error != noErr) {
        JXMPP_LOG(error) << "Unable to set connection to SSL context." << std::endl;
        sslContext_.reset();
    }
    ...
    if (error != noErr) {
        JXMPP_LOG(error) << "Unable to set kSSLSessionOptionBreakOnServerAuth on session." << std::endl;
        sslContext_.reset();
    }
}
...
void SecureTransportContext::setState(State newState) {
    JXMPP_LOG(debug) << "Switch state from " << stateToString(state_) << " to " << stateToString(newState) << "." << std::endl;
    state_ = newState;
}

void SecureTransportContext::connect() {
    JXMPP_LOG_ASSERT(state_ == None, error) << "current state '" << stateToString(state_) << " invalid." << std::endl;
    if (clientCertificate_) {
        ...
        if (certs) {
            ...
            if (result != noErr) {
                JXMPP_LOG(error) << "SSLSetCertificate failed with error " << result << "." << std::endl;
            }
        }
    }
    processHandshake();
...
void SecureTransportContext::processHandshake() {
    JXMPP_LOG_ASSERT(state_ == None || state_ == Handshake, error) << "current state '" << stateToString(state_) << " invalid." << std::endl;
    OSStatus error = SSLHandshake(sslContext_.get());
    if (error == errSSLWouldBlock) {
        setState(Handshake);
    }
    else if (error == noErr) {
        JXMPP_LOG(debug) << "TLS handshake successful." << std::endl;
        setState(HandshakeDone);
        onConnected();
    }
    else if (error == errSSLPeerAuthCompleted) {
        JXMPP_LOG(debug) << "Received server certificate. Start verification." << std::endl;
        setState(Handshake);
        verifyServerCertificate();
    }
    else {
        JXMPP_LOG(debug) << "Error returned from SSLHandshake call is " << error << "." << std::endl;
...
void SecureTransportContext::verifyServerCertificate() {
    ...
    switch(trustResult) {
        case kSecTrustResultUnspecified:
            JXMPP_LOG(warning) << "Successful implicit validation. Result unspecified." << std::endl;
            break;
        case kSecTrustResultProceed:
            JXMPP_LOG(warning) << "Validation resulted in explicitly trusted." << std::endl;
            break;
        case kSecTrustResultRecoverableTrustFailure:
            JXMPP_LOG(warning) << "recoverable trust failure" << std::endl;
...
void SecureTransportContext::handleDataFromNetwork(const SafeByteArray& data) {
    ...
    JXMPP_LOG_ASSERT(state_ == HandshakeDone || state_ == Handshake, error) << "current state '" << stateToString(state_) << " invalid." << std::endl;
        ...
        case HandshakeDone:
            while (error == noErr) {
                ...
                if (error == noErr) {
                    ...
                    JXMPP_LOG(error) << "SSLRead failed with error " << error << ", read bytes: " << bytesRead << "." << std::endl;
                    fatalError(std::make_shared<TLSError>(), std::make_shared<CertificateVerificationError>());
                    return;
                }
            ...
            break;
        case Error:
            JXMPP_LOG(debug) << "Igoring received data in error state." << std::endl;
            break;
    }
}
...
void SecureTransportContext::handleDataFromApplication(const SafeByteArray& data) {
    ...
    switch(error) {
        case errSSLWouldBlock:
            JXMPP_LOG(warning) << "Unexpected because the write callback does not block." << std::endl;
            return;
        case errSSLClosedGraceful:
        case noErr:
            return;
        default:
            JXMPP_LOG(warning) << "SSLWrite returned error code: " << error << ", processed bytes: " << processedBytes << std::endl;
...
std::vector<Certificate::ref> SecureTransportContext::getPeerCertificateChain() const {
    ...
    if (sslContext_) {
            ...
            if (error == noErr) {
                ...
                JXMPP_LOG(warning) << "Failed to obtain peer trust structure; error = " << error << "." << std::endl;
            }
    }
...
ByteArray SecureTransportContext::getFinishMessage() const {
    JXMPP_LOG(warning) << "Access to TLS handshake finish message is not part of OS X Secure Transport APIs." << std::endl;
    return ByteArray();
}
...
std::shared_ptr<CertificateVerificationError> SecureTransportContext::CSSMErrorToVerificationError(OSStatus resultCode) {
    ...
    switch(resultCode) {
        ...
        case CSSMERR_TP_VERIFY_ACTION_FAILED:
            JXMPP_LOG(debug) << "CSSM result code: CSSMERR_TP_VERIFY_ACTION_FAILED" << std::endl;
            break;
        case CSSMERR_APPLETP_INCOMPLETE_REVOCATION_CHECK:
            JXMPP_LOG(debug) << "CSSM result code: CSSMERR_APPLETP_INCOMPLETE_REVOCATION_CHECK" << std::endl;
            if (checkCertificateRevocation_) {
                error = std::make_shared<CertificateVerificationError>(CertificateVerificationError::RevocationCheckFailed);
            }
            break;
        case CSSMERR_APPLETP_OCSP_UNAVAILABLE:
            JXMPP_LOG(debug) << "CSSM result code: CSSMERR_APPLETP_OCSP_UNAVAILABLE" << std::endl;
            if (checkCertificateRevocation_) {
                error = std::make_shared<CertificateVerificationError>(CertificateVerificationError::RevocationCheckFailed);
            }
            break;
        case CSSMERR_APPLETP_SSL_BAD_EXT_KEY_USE:
            JXMPP_LOG(debug) << "CSSM result code: CSSMERR_APPLETP_SSL_BAD_EXT_KEY_USE" << std::endl;
            error = std::make_shared<CertificateVerificationError>(CertificateVerificationError::InvalidPurpose);
            break;
        default:
            JXMPP_LOG(warning) << "unhandled CSSM error: " << resultCode << ", CSSM_TP_BASE_TP_ERROR: " << CSSM_TP_BASE_TP_ERROR << std::endl;
