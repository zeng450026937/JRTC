...
OpenSSLContext::OpenSSLContext() : state_(Start), context_(0), handle_(0), readBIO_(0), writeBIO_(0) {
    ...
    // Load system certs
#if defined(JXMPP_PLATFORM_WINDOWS)
    X509_STORE* store = SSL_CTX_get_cert_store(context_);
    HCERTSTORE systemStore = CertOpenSystemStore(0, _TEXT("ROOT"));
    if (systemStore) {
...
#elif !defined(JXMPP_PLATFORM_MACOSX)
    SSL_CTX_set_default_verify_paths(context_);
#elif defined(JXMPP_PLATFORM_MACOSX) && !defined(JXMPP_PLATFORM_IPHONE)
