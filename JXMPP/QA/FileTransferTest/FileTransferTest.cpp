...
class FileTransferTest {
    public:
        FileTransferTest(int senderCandidates, int receiverCandidates) : senderCandidates_(senderCandidates), senderError_(FileTransferError::UnknownError), senderIsDone_(false), receiverCandidates_(receiverCandidates), receiverError_(FileTransferError::UnknownError), receiverIsDone_(false) {
            sender_ = std::make_shared<Client>(JID(getenv("JXMPP_FILETRANSFERTEST_JID")), getenv("JXMPP_FILETRANSFERTEST_PASS"), networkFactories.get());
            ...
            receiver_ = std::make_shared<Client>(JID(getenv("JXMPP_FILETRANSFERTEST2_JID")), getenv("JXMPP_FILETRANSFERTEST2_PASS"), networkFactories.get());
...
int main(int argc, char** argv) {
    ...
    if (argc == 1) {
        if (getenv("JXMPP_FILETRANSFERTEST_CONFIG")) {
            // test configuration described in JXMPP_FILETRANSFERTEST_CONFIG environment variable, e.g. "1:1|2:2"
            std::vector<std::string> configurations;
            std::string configs_env = std::string(getenv("JXMPP_FILETRANSFERTEST_CONFIG"));
