...
void CoreClient::connect(const ClientOptions& o) {
    ...
    switch (o.proxyType) {
        case ClientOptions::NoProxy:
            JXMPP_LOG(debug) << " without a proxy" << std::endl;
            break;
        case ClientOptions::SystemConfiguredProxy:
            JXMPP_LOG(debug) << " with a system configured proxy" << std::endl;
            if (systemSOCKS5Proxy.isValid()) {
                JXMPP_LOG(debug) << "Found SOCK5 Proxy: " << systemSOCKS5Proxy.getAddress().toString() << ":" << systemSOCKS5Proxy.getPort() << std::endl;
            ...
            if (systemHTTPConnectProxy.isValid()) {
                JXMPP_LOG(debug) << "Found HTTPConnect Proxy: " << systemHTTPConnectProxy.getAddress().toString() << ":" << systemHTTPConnectProxy.getPort() << std::endl;
        ...
        case ClientOptions::SOCKS5Proxy: {
            JXMPP_LOG(debug) << " with manual configured SOCKS5 proxy" << std::endl;
            ...
            JXMPP_LOG(debug) << "Proxy: " << proxyHostname << ":" << proxyPort << std::endl;
        ...
        case ClientOptions::HTTPConnectProxy: {
            JXMPP_LOG(debug) << " with manual configured HTTPConnect proxy" << std::endl;
    ...
    if (options.boshURL.isEmpty()) {
        ...
        if (certificate_ && !certificate_->isNull()) {
            JXMPP_LOG(debug) << "set certificate" << std::endl;
...
void CoreClient::sendData(const std::string& data) {
    if (!sessionStream_) {
        JXMPP_LOG(warning) << "Client: Trying to send data while disconnected." << std::endl;
        return;
    }
...
void CoreClient::forceReset() {
    if (connector_) {
        JXMPP_LOG(warning) << "Client not disconnected properly: Connector still active" << std::endl;
        resetConnector();
    }
    if (sessionStream_ || connection_) {
        JXMPP_LOG(warning) << "Client not disconnected properly: Session still active" << std::endl;
        resetSession();
    }
}
