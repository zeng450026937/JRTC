...
#ifdef JXMPP_PLATFORM_WIN32
#include <JXMPP/Base/WindowsRegistry.h>
...
if (WindowsRegistry::isFIPSEnabled()) {
    JXMPP_LOG(info) << "Windows is running in FIPS-140 mode. Some authentication methods will be unavailable." << std::endl;
}
#endif
}
    ...
    if (std::shared_ptr<Stanza> stanza = std::dynamic_pointer_cast<Stanza>(element)) {
        ...
        if (stanzaAckRequester_) {
            if (ack->isValid()) {
                ...
                JXMPP_LOG(warning) << "Got invalid ack from server";
            }
        }
        else {
            JXMPP_LOG(warning) << "Ignoring ack";
        }
    }
...
void ClientSession::handleStreamClosed(std::shared_ptr<JXMPP::Error> streamError) {
...
void ClientSession::handleStreamShutdownTimeout() {
    handleStreamClosed(std::shared_ptr<JXMPP::Error>());
}
...
void ClientSession::finish() {
    if (state != State::Finishing && state != State::Finished) {
        ...
        JXMPP_LOG(warning) << "Session already finished or finishing." << std::endl;
    }
}

void ClientSession::finishSession(Error::Type error) {
    finishSession(std::make_shared<JXMPP::ClientSession::Error>(error));
}

void ClientSession::finishSession(std::shared_ptr<JXMPP::Error> error) {
    if (!error_) {
        error_ = error;
    }
    else {
        JXMPP_LOG(warning) << "Session finished twice" << std::endl;
    }
