        ...
        if(result == ERROR_SUCCESS) {
            ...
            for(auto&& proxy : proxies) {
                if(proxy.find('=') != std::string::npos) {
                    ...
                    JXMPP_LOG(debug) << "Found proxy: " << protocolAndProxy.first << " => " << protocolAndProxy.second << std::endl;
...
HostAddressPort WindowsProxyProvider::getAsHostAddressPort(std::string proxy) {
    ...
    try {
        ...
        JXMPP_LOG(error) << "Exception occured while parsing windows proxy \"getHostAddressPort\"." << std::endl;
    }
