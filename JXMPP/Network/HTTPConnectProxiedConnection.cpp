...
void HTTPConnectProxiedConnection::initializeProxy() {
    ...
    append(data, createSafeByteArray("\r\n"));
    JXMPP_LOG(debug) << "HTTP Proxy send headers: " << byteArrayToString(ByteArray(data.begin(), data.end())) << std::endl;
    write(data);
}
...
void HTTPConnectProxiedConnection::handleProxyInitializeData(std::shared_ptr<SafeByteArray> data) {
    ...
    if (tmp.size() > 1) {
        try {
            ...
            JXMPP_LOG(debug) << "Proxy Status: " << status << std::endl;
            if (status / 100 == 2) { // all 2XX states are OK
                setProxyInitializeFinished(true);
            }
            else {
                JXMPP_LOG(debug) << "HTTP Proxy returned an error: " << httpResponseBuffer_ << std::endl;
                setProxyInitializeFinished(false);
            }
        }
        catch (boost::bad_lexical_cast&) {
            JXMPP_LOG(warning) << "Unexpected response: " << tmp[1] << std::endl;
            setProxyInitializeFinished(false);
        }
    }
    else {
