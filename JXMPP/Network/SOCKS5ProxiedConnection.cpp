...
void SOCKS5ProxiedConnection::handleProxyInitializeData(std::shared_ptr<SafeByteArray> data) {
    ...
    if (proxyState_ == ProxyAuthenticating) {
        JXMPP_LOG(debug) << "ProxyAuthenticating response received, reply with the connect BYTEs" << std::endl;
            ...
            switch(choosenMethod) { // use the correct Method
                case 0x00:
                    try {
                        ...
                        JXMPP_LOG(error) << "exception caught" << std::endl;
                    }
                    write(socksConnect);
                    break;
                default:
        ...
        JXMPP_LOG(debug) << "Connect response received, check if successfully." << std::endl;
        JXMPP_LOG(debug) << "Errorbyte: 0x" << std::hex << static_cast<int> ((*data)[1]) << std::dec << std::endl;
        /*
        ...
        */
        if ((*data)[0] == 0x05 && (*data)[1] == 0x0) {
            JXMPP_LOG(debug) << "Successfully connected the server via the proxy." << std::endl;
            setProxyInitializeFinished(true);
        }
        else {
            JXMPP_LOG(error) << "SOCKS Proxy returned an error: " << std::hex << (*data)[1] << std::endl;
            setProxyInitializeFinished(false);
        }
    }
}
