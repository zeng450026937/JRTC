#include <JXMPP/SASL/WindowsGSSAPIClientAuthenticator.h>

#include <JXMPP/Base/Log.h>
#include <JXMPP/Base/String.h>
#include <JXMPP/SASL/WindowsAuthentication.h>
#include <JXMPP/SASL/WindowsServicePrincipalName.h>
...
boost::optional<SafeByteArray> WindowsGSSAPIClientAuthenticator::getResponse() const {
    JXMPP_LOG(debug) << "response_.size(): " << response_.size() << std::endl;
    return response_;
}
...
bool WindowsGSSAPIClientAuthenticator::setChallenge(const boost::optional<ByteArray>& challengeData) {
    ...
    if (step_ == BuildingSecurityContext) {
        ...
        if (!challengeData) {
            JXMPP_LOG(debug) << "Empty message received from the server" << std::endl;
            error_ = true;
            return false;
        }
        ...
        if (challenge.size() != 4) {
            JXMPP_LOG(debug) << "Token received from the server of incorrect length: " << challenge.size() << std::endl;
            error_ = true;
            return false;
        }
        ...
        if (serverSecurityLayer == 0) {
            JXMPP_LOG(debug) << "Server supports unknown security layer, assuming no security layer" << std::endl;
            serverSecurityLayer = SECURITY_LAYER_NONE;
        }
        else if (serverSecurityLayer == SECURITY_LAYER_NONE) {
            JXMPP_LOG(debug) << "Server supports no security layer" << std::endl;
        }
        else {
            JXMPP_LOG(debug) << "Server supports security layer" << std::endl;
        }
        ...
        if ((serverSecurityLayer == SECURITY_LAYER_NONE) && (serverMaximumBuffer != 0)) {
            JXMPP_LOG(debug) << "Server supports no security layer but has maximum buffer size" << serverMaximumBuffer << std::endl;
            error_ = true;
            return false;
        }
...
void WindowsGSSAPIClientAuthenticator::buildSecurityContext(const boost::optional<ByteArray>& inputToken) {
    ...
    if (contextSupported & ISC_REQ_MUTUAL_AUTH == 0) {
        JXMPP_LOG(debug) << "Mutual authentication not supported" << std::endl;
        error_ = true;
        return;
    }
    ...
    userName_ = convertWStringToString(names.sUserName);
#else
    userName_ = names.sUserName;
#endif
    JXMPP_LOG(debug) << "User name: " << userName_ << std::endl;

    std::size_t position = userName_.find("\\");
    clientName_ = userName_.substr(position + 1);
    JXMPP_LOG(debug) << "Client name: " << clientName_ << std::endl;

    serverName_ = userName_.substr(0, position);
    JXMPP_LOG(debug) << "Server name: " << serverName_ << std::endl;
