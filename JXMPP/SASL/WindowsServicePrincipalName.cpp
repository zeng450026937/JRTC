...
std::string WindowsServicePrincipalName::toString() {
    ...
    JXMPP_LOG(debug) << "SPN: " << str << std::endl;
    return str;
}
...
#ifdef UNICODE
    JXMPP_LOG(debug) << "UNICODE is defined" << std::endl;
#else
    JXMPP_LOG(debug) << "UNICODE is not defined" << std::endl;
#endif

    JXMPP_LOG(debug) << "serviceClass_: " << convertWStringToString(serviceClass_.c_str()) << std::endl;
    JXMPP_LOG(debug) << "serviceName_: " << convertWStringToString(serviceName_.c_str()) << std::endl;
    JXMPP_LOG(debug) << "instanceName_: " << convertWStringToString(instanceName_.c_str()) << std::endl;
    JXMPP_LOG(debug) << "referrer_: " << convertWStringToString(referrer_.c_str()) << std::endl;
    JXMPP_LOG(debug) << "instancePort_: " << instancePort_ << std::endl;
    JXMPP_LOG(debug) << "length: " << *length << std::endl;
