...
void CombinedAvatarProvider::handleAvatarChanged(const JID& jid) {
    ...
    if (newHash != oldHash) {
        JXMPP_LOG(debug) << "Avatar changed: " << jid << ": " << oldHash << " -> " << (newHash ? newHash.get() : "NULL") << std::endl;
        onAvatarChanged(jid);
    }
}

boost::optional<std::string> CombinedAvatarProvider::getCombinedAvatarAndCache(const JID& jid) const {
    JXMPP_LOG(debug) << "JID: " << jid << std::endl;
    boost::optional<std::string> hash;
    for (size_t i = 0; i < providers.size() && !hash; ++i) {
        hash = providers[i]->getAvatarHash(jid);
        JXMPP_LOG(debug) << "Provider " << providers[i] << ": " << (hash ? hash.get() : "NULL") << std::endl;
    }
    if (hash) {
        avatars[jid] = *hash;
    } else {
